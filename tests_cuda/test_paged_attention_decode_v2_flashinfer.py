...
def _require_cuda() -> None:
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required for paged attention decode v2 test.")
...
def _require_flashinfer() -> None:
    try:
        ...
        pytest.skip("flashinfer is required for paged attention decode v2 test.")
...
def _ref_paged_attention_decode(
    ...
    # Simple reference (same as test_paged_attention_decode.py).
    B, H, D = query.shape
...
def test_paged_attention_decode_v2_flashinfer_matches_reference():
    _require_cuda()
    _require_flashinfer()

    from rec_llm_kernels.ops import paged_attention_decode_v2_flashinfer
    ...
    out = paged_attention_decode_v2_flashinfer(
        query, key_cache, value_cache, block_tables, context_lens, scale, backend="auto"
    )
